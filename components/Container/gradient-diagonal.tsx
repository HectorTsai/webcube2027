...
export default function GradientDiagonalContainer({
  ...
  // 完整類別（結構 + 顏色）
  const activeFullClasses = `${baseClasses} bg-gradient-to-br from-${color} to-${color}-50 text-${color}-content ${hover ? `hover:to-${color}-30` : ''}`;
  const inactiveFullClasses = `${baseClasses} bg-gradient-to-br from-base-70 to-base-30 text-base-content ${hover ? `hover:to-base-10` : ''}`;
