...
export default function GradientRightContainer({
  ...
  // 完整類別（結構 + 顏色）
  const activeFullClasses = `${baseClasses} bg-gradient-to-l from-${color} to-${color}-50 text-${color}-content `;
  const activeHoverClasses = `${baseClasses} bg-gradient-to-l from-${color} to-${color}-30 text-${color}-content `;
  const inactiveFullClasses = `${baseClasses} bg-gradient-to-l from-base-70 to-base-30 text-base-content `;
  const inactiveHoverClasses = `${baseClasses} bg-gradient-to-l from-base-70 to-base-10 text-base-content `;
  ...
  return <div class={`${classes} bg-gradient-to-l from-${colorFrom} to-${colorTo}`} style={{ width: widthStyle, height: heightStyle }} {...restProps}>{processedChildren}</div>;
}