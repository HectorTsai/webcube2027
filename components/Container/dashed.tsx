...
export default function DashedContainer({
  ...
  // 完整類別（結構 + 顏色）
  const activeFullClasses = `${baseClasses} bg-base border-2 border-dashed border-${color} text-${color} ${hover ? `hover:bg-${color}-70 hover:text-${color}-content` : ''}`;
  const inactiveFullClasses = `${baseClasses} bg-base border-2 border-dashed border-base-50 text-base-content ${hover ? `hover:bg-base-70` : ''}`;
