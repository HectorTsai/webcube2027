...
export default function DashedContainer({
  ...
  // 完整類別（結構 + 顏色）
  const activeFullClasses = `${baseClasses} bg-base border-2 border-dashed border-${color} text-${color} `;
  const activeHoverClasses = `${baseClasses} bg-${color}-70 border-2 border-dashed border-${color} text-${color}-content `;
  const inactiveFullClasses = `${baseClasses} bg-base border-2 border-dashed border-base-50 text-base-content `;
  const inactiveHoverClasses = `${baseClasses} bg-base-70 border-2 border-dashed border-base-50 text-base-content `;
  ...
  const finalClasses = [
    ...
    gapClasses[gap],
    "bg-base",
    `border-2 border-dashed border-${colorPrefix}`,
    `text-${textColor}`,
