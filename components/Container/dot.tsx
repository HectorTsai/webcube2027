...
import { paddingClasses, marginClasses, alignClasses, justifyClasses, gapClasses, roundedClasses, directionClasses, color2TextColor, adjustColorLightOrOpacity } from "../classes.ts";
...
export default function DotContainer({
  ...
  // 完整類別（結構 + 顏色）
  const activeFullClasses = `${baseClasses} bg-base border-2 border-dotted border-${color} text-${color} `;
  const activeHoverClasses = `${baseClasses} bg-${color}-70 border-2 border-dotted border-${color} text-${color}-content `;
  const inactiveFullClasses = `${baseClasses} bg-base border-2 border-dotted border-base-50 text-base-content `;
