...
export default function GhostContainer({
  ...
  // 完整類別（結構 + 顏色）
  const activeFullClasses = `${baseClasses} bg-transparent text-${color} `;
  const activeHoverClasses = `${baseClasses} bg-${color}-70 text-${color}-content `;
  const inactiveFullClasses = `${baseClasses} bg-transparent text-base-content `;
  const inactiveHoverClasses = `${baseClasses} bg-base-70 text-base-content `;
