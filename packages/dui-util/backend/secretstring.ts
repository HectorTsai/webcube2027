...
export default class SecretString {
  ...
  /** Single-flight guard: concurrent process() calls share one promise */
  private _processing: Promise<void> | null = null;

  public constructor(
    options?: { plainText?: string; cipherText?: string | undefined | null },
  ) {
   ...
   *
   * Concurrent calls share a single in-flight promise to avoid
   * duplicate encrypt/decrypt CPU overhead.
   */
  public async process(): Promise<void> {
    if (this._processing) return this._processing;

    this._processing = (async () => {
      try {
        if (!this.ciphertext && this.plaintext) {
          this.ciphertext = await encrypt(this.plaintext);
        }
        if (!this.plaintext && this.ciphertext) {
          this.plaintext = await decrypt(this.ciphertext);
        }
      } catch (error) {
  ...
  public set PlainText(text: string) {
    this.plaintext = text;
    this.ciphertext = undefined;
  }
  ...
  public set CipherText(text: string) {
    this.ciphertext = text;
    this.plaintext = undefined;
  }
