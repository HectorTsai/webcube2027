...
const PROMPT_HARDWARE = `你是電腦硬體規格專家。根據使用者輸入的 CPU 或硬體描述，補齊完整的硬體規格。
...
const PROMPT_TRANSLATE = `你是專業的多國語言翻譯專家。將使用者提供的欄位內容逐一翻譯成目標語言。
...
const PROMPT_CONTEXT = `你是內容建議專家。根據使用者已填寫的欄位，為空白或需要協助的欄位提供建議內容。
