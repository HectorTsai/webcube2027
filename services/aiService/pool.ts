...
import { AIProvider, AI聊天訊息, AI回應, AI能力, AITaskConfig } from './provider/adapter.ts';
...
export class AIPoolManager {
  ...
  private async 建立Provider(server: AI伺服器, 模型名稱: string): Promise<AIProvider | null> {
    ...
    if (PROVIDER_OPENAI_COMPATIBLE.has(p)) {
      return new OpenAIProvider(apiKeyString, 模型名稱, server.url);
    }
    if (PROVIDER_OLLAMA.has(p)) {
      return new OllamaProvider(server.url, 模型名稱, apiKeyString);
    }
    if (PROVIDER_ANTHROPIC.has(p)) {
      return new AnthropicProvider(apiKeyString, 模型名稱);
    }
    if (PROVIDER_GEMINI.has(p)) {
      return new GeminiProvider(apiKeyString, 模型名稱);
    }
    return null;
  }
