import { 權限, 資料 } from "../index.ts"
import { MultilingualString } from "@dui/smartmultilingual"

const _DEFAULT_STRINGS = {
  名稱: { en: "classic blue", "zh-tw": "經典藍", vi: "màu xanh cổ điển" },
  描述: {
    en: "A blue tone, giving a feeling of calm, gentleness and comfort.",
    "zh-tw": "一種藍色調，給人冷靜、柔和舒服的感覺",
    vi: "Một tông màu xanh, mang lại cảm giác bình tĩnh, nhẹ nhàng và thoải mái.",
  },
}

const DEFAULT_COLORS = {
  主色: "59.67% 0.221 258.03",
  主色內容: "0% 0 0",
  次色: "39.24% 0.128 255",
  次色內容: "100% 0 0",
  強調色: "77.86% 0.1489 226.0173",
  強調色內容: "0% 0 0",
  中性色: "35.5192% .032071 262.988584",
  中性色內容: "100% 0 0",
  背景1: "100% 0 0",
  背景1內容: "0% 0 0",
  背景2: "93% 0 0",
  背景2內容: "0% 0 0",
  背景3: "88% 0 0",
  背景3內容: "0% 0 0",
  背景內容: "35.5192% .032071 262.988584",
  資訊色: "71.17% 0.166 241.15",
  資訊色內容: "100% 0 0",
  成功色: "60.9% 0.135 161.2",
  成功色內容: "100% 0 0",
  警告色: "73% 0.19 52",
  警告色內容: "0% 0 0",
  錯誤色: "57.3% 0.234 28.28",
  錯誤色內容: "100% 0 0",
}

export default class 配色 implements 資料 {
  public id: string
  public 名稱: MultilingualString
  public 描述: MultilingualString
  public 主色: string
  public 主色內容: string
  public 次色: string
  public 次色內容: string
  public 強調色: string
  public 強調色內容: string
  public 中性色: string
  public 中性色內容: string
  public 背景1: string
  public 背景1內容: string
  public 背景2: string
  public 背景2內容: string
  public 背景3: string
  public 背景3內容: string
  public 背景內容: string
  public 資訊色: string
  public 資訊色內容: string
  public 成功色: string
  public 成功色內容: string
  public 警告色: string
  public 警告色內容: string
  public 錯誤色: string
  public 錯誤色內容: string
  public 售價: number

  public constructor(
    data: Record<string, unknown> = {},
    _權限設定: 權限 = { 讀: true, 寫: true, 刪除: true },
  ) {
    this.id = data?.id as string || "配色:預設"
    this.名稱 = new MultilingualString(data?.名稱 as Record<string, string> | undefined)
    this.描述 = new MultilingualString(data?.描述 as Record<string, string> | undefined)
    this.主色 = (data?.主色 as string) ?? DEFAULT_COLORS.主色
    this.主色內容 = (data?.主色內容 as string) ?? DEFAULT_COLORS.主色內容
    this.次色 = (data?.次色 as string) ?? DEFAULT_COLORS.次色
    this.次色內容 = (data?.次色內容 as string) ?? DEFAULT_COLORS.次色內容
    this.強調色 = (data?.強調色 as string) ?? DEFAULT_COLORS.強調色
    this.強調色內容 = (data?.強調色內容 as string) ?? DEFAULT_COLORS.強調色內容
    this.中性色 = (data?.中性色 as string) ?? DEFAULT_COLORS.中性色
    this.中性色內容 = (data?.中性色內容 as string) ?? DEFAULT_COLORS.中性色內容
    this.背景1 = (data?.背景1 as string) ?? DEFAULT_COLORS.背景1
    this.背景1內容 = (data?.背景1內容 as string) ?? DEFAULT_COLORS.背景1內容
    this.背景2 = (data?.背景2 as string) ?? DEFAULT_COLORS.背景2
    this.背景2內容 = (data?.背景2內容 as string) ?? DEFAULT_COLORS.背景2內容
    this.背景3 = (data?.背景3 as string) ?? DEFAULT_COLORS.背景3
    this.背景3內容 = (data?.背景3內容 as string) ?? DEFAULT_COLORS.背景3內容
    this.背景內容 = (data?.背景內容 as string) ?? DEFAULT_COLORS.背景內容
    this.資訊色 = (data?.資訊色 as string) ?? DEFAULT_COLORS.資訊色
    this.資訊色內容 = (data?.資訊色內容 as string) ?? DEFAULT_COLORS.資訊色內容
    this.成功色 = (data?.成功色 as string) ?? DEFAULT_COLORS.成功色
    this.成功色內容 = (data?.成功色內容 as string) ?? DEFAULT_COLORS.成功色內容
    this.警告色 = (data?.警告色 as string) ?? DEFAULT_COLORS.警告色
    this.警告色內容 = (data?.警告色內容 as string) ?? DEFAULT_COLORS.警告色內容
    this.錯誤色 = (data?.錯誤色 as string) ?? DEFAULT_COLORS.錯誤色
    this.錯誤色內容 = (data?.錯誤色內容 as string) ?? DEFAULT_COLORS.錯誤色內容
    this.售價 = (data?.售價 as number) ?? 0
  }

  public toJSON(): Record<string, unknown> {
    return {
      id: this.id,
      名稱: this.名稱.toJSON(),
      描述: this.描述.toJSON(),
      主色: this.主色,
      主色內容: this.主色內容,
      次色: this.次色,
      次色內容: this.次色內容,
      強調色: this.強調色,
      強調色內容: this.強調色內容,
      中性色: this.中性色,
      中性色內容: this.中性色內容,
      背景1: this.背景1,
      背景1內容: this.背景1內容,
      背景2: this.背景2,
      背景2內容: this.背景2內容,
      背景3: this.背景3,
      背景3內容: this.背景3內容,
      背景內容: this.背景內容,
      資訊色: this.資訊色,
      資訊色內容: this.資訊色內容,
      成功色: this.成功色,
      成功色內容: this.成功色內容,
      警告色: this.警告色,
      警告色內容: this.警告色內容,
      錯誤色: this.錯誤色,
      錯誤色內容: this.錯誤色內容,
      售價: this.售價,
    }
  }
}
