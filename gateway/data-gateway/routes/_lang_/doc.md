...
| PUT | `/api/lX/` | **批次整筆更新（upsert）**，body 為 JSON 陣列，每筆依 composite ID 自動決定 collection/model，可跨不同 collection（適合 pool flush） |
| PATCH | `/api/lX/` | **批次部分更新**，body 為 JSON 陣列，每筆依 composite ID 自動決定 collection/model |
| DELETE | `/api/lX/` | **批次刪除**，body 為 id 字串陣列（或含 `id` 之物件陣列），不限定同 collection |
...
```json
{
  ...
  "data": {
    ...
    "失敗": ["bad:id"],
    "失敗原因": { "bad:id": "無效的 composite ID：需 collection:model:nanoid" }
  }
}
```
