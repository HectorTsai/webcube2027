...
import { CubeUpgrader } from './task/cube-upgrader.ts';
...
export async function 處理AI請求(c: Context): Promise<Response> {
  ...
  try {
    switch (true) {
      ...
      // ── 方塊自動升級 ──
      case method === 'POST' && aiPath === 'internal/upgrade-cubes': {
        const upgrader = new CubeUpgrader(c);
        const result = await upgrader.升級舊方塊();
        return c.json({ success: true, data: result });
      }
