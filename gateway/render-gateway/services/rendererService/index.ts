...
async function 生成完整HTML(
  ...
  // 2. 動態收集本頁所有方塊宣告的全域 Alpine Store
  const 所需Stores = c.get("page_required_stores") as Set<string> | undefined;
  const storeInitScript = 所需Stores && 所需Stores.size > 0
    ? `<script>
  document.addEventListener('alpine:init', () => {
    ${Array.from(所需Stores).map(s => `Alpine.store('${s}', Alpine.store('${s}') || {});`).join('\n    ')}
  });
</script>
  `
    : '';

  // 3. 提取真實的色彩數據 (保底走經典藍數值)
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        /* 🛡️ x-cloak 安全閘：在 Alpine 完全初始化前隱藏所有帶 x-cloak 的元素 */
