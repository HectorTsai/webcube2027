...
// 處理取得所有風格（列表，供 AI 參考）
async function 處理取得所有風格(c: Context): Promise<Response> {
  try {
    const 結果 = await 資料池.查詢列表<風格>('風格', 100, 0);
    const language = await 取得語言(c);
    ...
    await error('風格 API', `取得所有風格失敗: ${錯誤}`);
    return c.json({ success: false, error: { code: 'INTERNAL_ERROR', message: '取得風格列表失敗' } }, 500);
  }
}
