...
import { getDataGatewayUrl } from './config.ts';
...
export async function list<T>(
  ...
  const base = await getDataGatewayUrl();
...
export async function create<T>(
  ...
  const base = await getDataGatewayUrl();
  const res = await fetch(`${base}/api/${collection}/${model}`, {
    method: 'POST',
...
export async function del(id: string): Promise<void> {
  const base = await getDataGatewayUrl();
  const res = await fetch(`${base}/api/${id}`, { method: 'DELETE' });
